...
import {
	...
	discussionStore,
	useFakeUserInfo,
} from "@/components/plate-ui/block-discussion";
import { Button } from "@/components/ui/button";
import {
	type TComment,
	Comment,
	formatCommentDate,
} from "@/components/plate-ui/comment";
...
export const BlockSuggestionCard = ({
	...
	const generateStableKey = (
		...
		// Create a hash-like value from the text content
		const textHash = text
			.split("")
			.reduce((hash, char) => ((hash << 5) - hash + char.charCodeAt(0)) | 0, 0);

		return `${suggestionId}-${type}-${textHash}-${position}`;
	};
				...
				<div className="relative mt-1 mb-4 pl-[32px]">
					<div className="flex flex-col gap-2">
						{suggestion.type === "remove" && (
							<React.Fragment>
								{suggestionText2Array(suggestion.text ?? "").map((text, i) => {
...
export const useResolveSuggestion = (
	...
	// Using optional chaining and providing a fallback empty array to avoid type errors
	const discussions = useStoreSelect(
		discussionStore,
		// @ts-ignore Missing type definitions for discussionStore
