...
import { type TComment, Comment } from "@/components/plate-ui/comment";
...
const BlockCommentsContent = ({
	...
	const suggestionsCount = resolvedSuggestion?.length ?? 0;
	...
	const selected =
		(resolvedDiscussions?.some((d) => d.id === activeCommentId) ?? false) ||
	...
	if (
		suggestionsCount + (resolvedDiscussions?.length ?? 0) === 0 &&
		!draftCommentNode
	)
