...
export const MediaPlaceholderElement = withHOC(
	...
	withRef<typeof PlateElement>(
		({ children, className, nodeProps, ...props }, ref) => {
				...
				onFilesSelected: ({ plainFiles: updatedFiles }) => {
					...
					if (restFiles.length > 0) {
						for (const file of restFiles) {
							// Insert each file as a media node using proper API
							void uploadFile(file).then((uploaded) => {
								if (uploaded) {
									...
									editor.tf.insertNodes(mediaNode);
								}
							});
						}
					}
			...
			// Memoize the values we need from element
			...
			// Create stable references to functions
			const removeUploadingFile = useCallback(
				(id: string) => api.placeholder.removeUploadingFile(id),
				[api.placeholder],
			);

			// This effect handles replacing the placeholder with the uploaded file
			useEffect(() => {
				if (!uploadedFile) return;

				const path = editor.api.findPath(elementValues.element);
				...
				editor.tf.withoutSaving(() => {
					...
					const node = {
						...
						isUpload: true,
						name:
							elementValues.mediaType === FilePlugin.key
								? uploadedFile.name
								: "",
						placeholderId: elementValues.id as string,
						type: elementValues.mediaType as string,
				...
				removeUploadingFile(elementValues.id as string);
			}, [uploadedFile, elementValues, removeUploadingFile, editor]);
